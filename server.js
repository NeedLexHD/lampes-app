...
app.post('/api/devices/:deviceId/commands', async (req, res) => {
  try {
    ...
    const stringToSign = [ACCESS_ID, token, t, 'POST', contentHash, '', `/v1.0/devices/${deviceId}/commands`].join('');
    const sign = crypto.createHmac('sha256', ACCESS_SECRET).update(stringToSign, 'utf8').digest('hex').toUpperCase();
